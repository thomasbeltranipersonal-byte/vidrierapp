...
// ─── ITEM PLANO (upload PDF/JPG) ────────────────────────────────────────────
const ItemCanvas=({value,onChange,label,itemIdx})=>{
  const [open,setOpen]=useState(!!(value&&value.length));
  const archivos=value||[];
  const commit=(arr)=>{onChange(arr);};

  const subirArchivo=(e)=>{
    Array.from(e.target.files).forEach(f=>{
      if(f.size>8*1024*1024){alert("Máx 8MB por archivo");return;}
      const r=new FileReader();
      r.onload=ev=>commit([...archivos,{data:ev.target.result,nombre:f.name,tipo:f.type,fecha:new Date().toISOString()}]);
      r.readAsDataURL(f);
    });
  };

  const eliminar=(i)=>commit(archivos.filter((_,idx)=>idx!==i));
  const ver=(a)=>window.open(a.data,"_blank");
  ...
  if(!open) return(
    <button onClick={()=>setOpen(true)} style={{width:"100%",marginTop:6,padding:"5px 0",background:"transparent",border:"1px dashed #21262d",borderRadius:6,color:"#4a6a7a",cursor:"pointer",fontSize:10,fontFamily:"monospace",display:"flex",alignItems:"center",justifyContent:"center",gap:5}}>
      📎 {archivos.length>0?`Plano adjunto (${archivos.length})`:"+ Adjuntar plano (PDF/JPG)"}
    </button>
  );

  return(
    <div style={{marginTop:8,background:"#0a1020",borderRadius:8,padding:10,border:"1px solid #1e3a5a"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
        <span style={{fontSize:10,fontWeight:700,color:"#5a8ab8",textTransform:"uppercase",letterSpacing:1}}>📐 Plano técnico</span>
        <div style={{display:"flex",gap:6}}>
          <label style={{padding:"3px 10px",borderRadius:6,border:"1px solid #1565C040",background:"#0a1828",color:"#64B5F6",cursor:"pointer",fontSize:11,fontFamily:"inherit",fontWeight:600}}>
            + Subir
            <input type="file" accept="image/*,application/pdf" multiple style={{display:"none"}} onChange={subirArchivo}/>
          </label>
          <button onClick={()=>setOpen(false)} style={{background:"none",border:"1px solid #21262d",color:"#3a6a9a",borderRadius:5,padding:"3px 8px",cursor:"pointer",fontSize:10,fontFamily:"inherit"}}>▲</button>
        </div>
      </div>

      {archivos.length===0&&(
        <div style={{textAlign:"center",padding:"20px 0",color:"#2a4a6a",fontSize:12}}>
          <div style={{fontSize:24,marginBottom:6}}>📎</div>
          Subí el plano en PDF o imagen (JPG, PNG)
        </div>
      )}
        ...
        {archivos.map((a,i)=>(
          <div key={i} style={{position:"relative",width:90}}>
            {a.tipo==="application/pdf"
              ?<div onClick={()=>ver(a)} style={{width:90,height:90,background:"#0a1828",borderRadius:7,border:"1px solid #1e3a5a",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",cursor:"pointer",gap:4}}>
                <span style={{fontSize:24}}>📄</span>
                <span style={{fontSize:8,color:"#5a8ab8",textAlign:"center",padding:"0 4px",wordBreak:"break-all"}}>{a.nombre.slice(0,20)}</span>
              </div>
              :<img src={a.data} alt="" style={{width:90,height:90,objectFit:"cover",borderRadius:7,border:"1px solid #1e3a5a",cursor:"pointer",display:"block"}} onClick={()=>ver(a)}/>
            }
            <button onClick={()=>eliminar(i)} style={{position:"absolute",top:-5,right:-5,width:18,height:18,borderRadius:"50%",background:"#7f2020",border:"none",color:"#fff",cursor:"pointer",fontSize:11,display:"flex",alignItems:"center",justifyContent:"center",lineHeight:1}}>✕</button>
            <div style={{fontSize:8,color:"#3a6a9a",marginTop:2,textAlign:"center",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{a.nombre}</div>
          </div>
        ))}
      </div>
    </div>
...
// getSVGForPDF — for backward compatibility with plano field (now returns empty)
...
const DocForm=({doc,modo,clientes,tiposVidrio,obsOpciones,serviciosOpciones,estados,stock=[],onSave,onClose,onConvertir})=>{
  ...
  const TABS_COT=[{id:"pedido",label:"📋 Pedido"}];
  const TABS_ORD=[{id:"pedido",label:"📋 Pedido"},{id:"produccion",label:"🏭 Producción"},{id:"instalacion",label:"🚚 Instalación"},{id:"actividad",label:"🕐 Actividad"}];
  ...
  const pdfEtiquetas=()=>{
    ...
    const labelHTML=etiquetas.map((e)=>`
      <div class="label">
        <div class="header">
          <img src="data:image/png;base64," style="width:28px;height:28px;object-fit:contain;display:block"/>
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .header{display:flex;align-items:center;gap:3mm;margin-bottom:1.5mm}
  ...
  // Render uploaded plano files (PDF/images) for PDF output
  const renderPlanoFiles=(files)=>{
    if(!files||!files.length) return "";
    const imgs=files.filter(f=>f.tipo&&f.tipo.startsWith("image/")).map(f=>`<img src="${f.data}" style="max-width:100%;max-height:180px;border-radius:4px;border:1px solid #1a4a6e;display:block;margin-bottom:6px"/>`).join("");
    const pdfs=files.filter(f=>f.tipo==="application/pdf").map(f=>`<div style="padding:6px 10px;background:#eef4ff;border:1px solid #1a4a6e;border-radius:4px;font-size:11px;color:#1a4a6e;margin-bottom:6px">📄 ${f.nombre}</div>`).join("");
    return imgs+pdfs;
  };
  const pdfTaller=()=>{
    const rows=form.items.map((it,i)=>{
      const itemSVG=renderPlanoFiles(it.plano||[]);
      return`
    ...
    const planoSVG=!hasItemPlanos?renderPlanoFiles(form.plano||[]):"";
    ...
    const procesoRows = (form.items||[]).map((it,i)=>{
      ...
      </tr>${planoHTML?`<tr><td colspan="${4+procesos.length}" style="padding:8px 12px;background:#f0f6ff;border:1px solid #dde8ff"><div style="font-size:9px;font-weight:700;color:#1a4a6e;margin-bottom:4px">📐 PLANO — ${it.nombre||it.tipo_vidrio||`Ítem ${i+1}`}</div>${planoHTML}</td></tr>`:""}`;
    }).join("");
...
function AppInner({ currentUser, onLogout }) {
  ...
  const navItems=[
    ...
    ...(!isLocal&&!isTaller?[{id:"reportes",label:"Reportes",icon:"optimize"}]:[]),
  ...
  const Cotizaciones=()=>{
    ...
    // Auto-cleanup: delete cotizaciones older than 30 days from papelera
    React.useEffect(()=>{
      const DAYS30=30*24*60*60*1000;
      const now=Date.now();
      cotizaciones.filter(c=>c.eliminada&&c.fecha_eliminacion&&(now-new Date(c.fecha_eliminacion).getTime())>DAYS30)
        .forEach(c=>fsDel("cotizaciones",c.id));
    },[cotizaciones.length]);

    const activas=cotizaciones.filter(c=>!c.eliminada);
    const enPapelera=cotizaciones.filter(c=>c.eliminada);

    const diasRestantes=(c)=>{
      if(!c.fecha_eliminacion) return 30;
      const elapsed=Date.now()-new Date(c.fecha_eliminacion).getTime();
      return Math.max(0,30-Math.floor(elapsed/86400000));
    };

    const restaurar=async(c)=>{
      const {eliminada,fecha_eliminacion,...resto}=c;
      await fsSet("cotizaciones",c.id,resto);
    };

    const lista=verPapelera?enPapelera:activas;
      ...
      <div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:22}}>
          <div>
            ...
            <p style={{margin:0,color:"#3a6a9a",fontSize:13}}>{activas.length} activas{enPapelera.length>0?` · ${enPapelera.length} en papelera`:""}</p>
          </div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            {enPapelera.length>0&&<button onClick={()=>setVerPapelera(v=>!v)} style={{padding:"6px 12px",borderRadius:7,border:`1px solid ${verPapelera?"#FFB74D":"#5a3a1a"}`,background:verPapelera?"#2a1500":"transparent",color:verPapelera?"#FFB74D":"#8a5a3a",cursor:"pointer",fontSize:11,fontFamily:"inherit",fontWeight:600}}>
            ...
            {!verPapelera&&<Btn small onClick={()=>setModal({type:"nueva_cotizacion"})}><Icon name="plus" size={14}/> Nueva Cotización</Btn>}
          </div>
        </div>

        {verPapelera&&<div style={{background:"#1a0a00",borderRadius:9,padding:"10px 14px",marginBottom:14,border:"1px solid #FFB74D30",fontSize:12,color:"#8a5a3a"}}>
          🗑 Papelera — las cotizaciones se eliminan definitivamente a los 30 días. Podés restaurarlas antes de eso.
        </div>}
        ...
        <div style={{display:"grid",gap:8}}>
          {lista.map(c=>{
            ...
            const dias=verPapelera?diasRestantes(c):null;
            return(
              <div key={c.id} style={{background:"#071220",borderRadius:11,padding:"12px 16px",border:`1px solid ${verPapelera?"#FFB74D20":"#0f2035"}`,display:"flex",alignItems:"center",gap:12,opacity:verPapelera?0.8:1}}>
                <div style={{background:"#0a1828",border:`1px solid ${verPapelera?"#FFB74D25":"#FFB74D25"}`,borderRadius:7,padding:"4px 10px",minWidth:105,textAlign:"center",flexShrink:0}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  {verPapelera&&<div style={{fontSize:11,color:dias<=5?"#ef9a9a":"#FFB74D",marginTop:2}}>⏳ Se elimina en {dias} día{dias!==1?"s":""}</div>}
                </div>
                {!verPapelera&&<BadgeCot estado={c.estado}/>}
                <div style={{display:"flex",gap:4}}>
                  {verPapelera
                    ?<><button onClick={()=>restaurar(c)} style={{padding:"5px 10px",borderRadius:7,border:"1px solid #26A69A40",background:"#0a1a10",color:"#26A69A",cursor:"pointer",fontSize:11,fontFamily:"inherit",fontWeight:600}}>↩ Restaurar</button>
                    <button onClick={async()=>{if(window.confirm("¿Eliminar definitivamente? No se puede deshacer."))fsDel("cotizaciones",c.id);}} style={{background:"none",border:"none",color:"#5a2a3a",cursor:"pointer",padding:6,display:"flex",borderRadius:6}}><Icon name="trash" size={15}/></button></>
                    ...
                    <button onClick={async()=>{
                      if(!window.confirm(`¿Enviar "${c.numero}" a la papelera?\nPodrás restaurarla dentro de 30 días.`))return;
                      await fsSet("cotizaciones",c.id,{...c,eliminada:true,fecha_eliminacion:new Date().toISOString()});
                    }} style={{background:"none",border:"none",color:"#5a2a3a",cursor:"pointer",padding:6,display:"flex",borderRadius:6}}><Icon name="trash" size={15}/></button></>
                  }
                </div>
              </div>
            );
          })}
          {!lista.length&&<div style={{textAlign:"center",padding:"44px 0",color:"#2a4a6a"}}>
            {verPapelera?<p style={{fontSize:14}}>La papelera está vacía</p>:<><p style={{marginTop:12,fontSize:14}}>No hay cotizaciones aún</p><Btn small onClick={()=>setModal({type:"nueva_cotizacion"})} style={{marginTop:8}}><Icon name="plus" size={14}/> Crear primera cotización</Btn></>}
          </div>}
        </div>
      </div>
    );
...
${bizFooter()}`;
  ...
  const Stock=()=>{
    ...
    const deleteItem=(id)=>{if(!window.confirm("¿Eliminar este ítem de stock?"))return;fsDel("stock_items",id);};
